...
Our target is to create biadjacency metrix for $T$ snapshots.
The biadjacency metrix $B$ hence will be a 3-dimensional matrix with shape $T \times n_1 \times n_2$.
...
After that, we create the cocluster relationship.
...
\begin{table}[H]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|m{0.1\textwidth}|m{0.4\textwidth}|m{0.2\textwidth}|m{0.1\textwidth}|m{0.1\textwidth}|}
			\hline
...
\begin{table}[H]
	\centering
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|m{0.1\textwidth}|m{0.3\textwidth}|m{0.3\textwidth}|m{0.1\textwidth}|m{0.1\textwidth}|}
			\hline
...
In this chapter we have described the process of creating a syntehsis dataset with predefined community structure.
